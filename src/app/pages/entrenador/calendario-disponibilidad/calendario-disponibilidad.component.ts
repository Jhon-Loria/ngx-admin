import { Component, OnInit } from '@angular/core';
import { NbCalendarRange, NbToastrService } from '@nebular/theme';

@Component({
  selector: 'ngx-calendario-disponibilidad',
  templateUrl: './calendario-disponibilidad.component.html',
  styleUrls: ['./calendario-disponibilidad.component.scss']
})
export class CalendarioDisponibilidadComponent implements OnInit {
  date = new Date();
  range: NbCalendarRange<Date> = { start: new Date(), end: new Date() };
  horariosDisponibles: string[] = [];
  selectedDate: Date = new Date();

  horarios = [
    '06:00', '07:00', '08:00', '09:00', '10:00', '11:00',
    '12:00', '13:00', '14:00', '15:00', '16:00', '17:00',
    '18:00', '19:00', '20:00', '21:00', '22:00'
  ];

  constructor(private toastrService: NbToastrService) {}

  ngOnInit(): void {
    this.horariosDisponibles = ['09:00', '10:00', '11:00', '16:00', '17:00', '18:00'];
  }

  toggleHorario(horario: string): void {
    const index = this.horariosDisponibles.indexOf(horario);
    if (index > -1) {
      this.horariosDisponibles.splice(index, 1);
    } else {
      this.horariosDisponibles.push(horario);
    }
  }

  isHorarioDisponible(horario: string): boolean {
    return this.horariosDisponibles.includes(horario);
  }

  guardarDisponibilidad(): void {
    this.toastrService.success('Disponibilidad guardada correctamente', 'Éxito');
  }
}
